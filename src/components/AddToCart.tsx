...
export function AddToCart({ kod, max }: { kod: string; max: number }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      >
        ...
        {full ? "Səbətdədir" : "Səbətə at"}
      </button>
      {(added || inCart > 0) && (
        <Link href="/sebet" className="text-center text-[14.5px] font-semibold text-emerald hover:text-emerald-dark">
          Səbətə keç ({inCart}) →
        </Link>
      )}
    </div>

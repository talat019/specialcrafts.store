 ...
 */
export default async function TestOdenisPage({
  params,
  ...
  async function pay() {
    ...
    redirect(L(l, `/odenis/netice/${order.reference}`));
  }

  async function fail() {
    "use server";
    await markOrderPaymentFailed(id, "legv");
    redirect(`${L(l, `/odenis/netice/${order.reference}`)}?hal=legv`);
  }
  ...
  return (
    <div className="mx-auto max-w-[560px] px-5 py-16 lg:py-24">
      <div className="rounded-2xl border-2 border-dashed border-gold bg-order-tint p-7">
        <p className="eyebrow text-gold-dark">{t.payment.testTitle}</p>
        <h1 className="mt-3 text-[28px] leading-tight">{t.payment.testHeading}</h1>
        <p className="mt-3 text-[15px] leading-relaxed text-ink-muted">{t.payment.testBody}</p>
      </div>

      <div className="mt-6 rounded-2xl border border-line bg-surface p-6">
        <div className="flex justify-between border-b border-line pb-3">
          <span className="text-ink-muted">{t.payment.order}</span>
